...
def main():
    ...
    if if_normal == 1:
        ...
        # u_train_phy = u_train.copy()
        u_test_phy = u_test.copy()

        # u_mean_train = u_train_reshape_phy.mean(axis=0)
        # u_std = u_train_reshape_phy.std(axis=0)
    ...
    if test_model == 0:
        ...
        for epoch in range(0, epochs):
            ...
            # MSE_err = np.mean( 
            #         np.mean( (y_pred.reshape(-1, num_pts) - u_test.reshape(-1, num_pts))**2, axis=1 ) )
        ...
        ## 
        # 
        # x_vis = np.loadtxt("canonical_uvc.txt")
        # x_vis_tensor = torch.tensor(x_vis, dtype=torch.float).to(device)
        # u_pred_train = to_numpy(model.forward(f_train_tensor, x_pace_train_tensor, x_vis_tensor))
        # save_id = 0
        # u_save = u_pred_train[save_id]*u_scale
        # np.savetxt("./vis/LATs_train_pred.txt", u_save.transpose())
